        ...
        <b-card :title="`Penyesuaian Stok Tanggal : ${tanggal}`">
          ...
          <b-table show-empty empty-text="Tidak ada data" class="position-relative" :items="persediaan" :fields="tableColumns">
            ...
            <template #cell(jumlah_fisik)="data">
              <span>
                <b-form-input type="number" min="0" v-model="data.item.jumlah_fisik" />
              </span>
            </template>
...
export default {
  ...
  methods: {
    ...
    perbedaan(data) {
      data.item.perbedaan = parseFloat(data.item.jumlah_fisik) - parseFloat(data.item.jumlah_tercatat)
      data.item.jenis = data.item.perbedaan < 0 ? 'KREDIT' : 'DEBIT'
      return data.item.perbedaan
    },
  ...
  setup() {
    ...
    const tableColumns = [
      ...
      { key: 'harga', label: 'harga perolehan', sortable: false },
