      ...
      <b-col lg="3" cols="12">
        <b-card>
          <b-form-group>
            ...
            <v-select v-model="supplier" placeholder="Cara Pembayaran" label="nama" :clearable="true" :options="listSupplier" />
          </b-form-group>
        </b-card>
      </b-col>
      <b-col lg="6" cols="12">
        <b-card title="Statistik Utang">
          <b-card-body>
            <b-row>
          ...
          <div class="mb-2">
            ...
            <b-row>
              <b-col cols="12" md="8" class="d-flex align-items-center justify-content-start mb-1 mb-md-0">
                ...
                <b-button variant="success" :to="{ name: 'keuangan-bulk-pembayaran-utang', params: { supplier: listSupplier } }">
                  Bulk Pembayaran
                </b-button>
              </b-col>
              <!-- Search -->
...
export default {
  ...
  watch: {
    dateFilter(x) {
      if (x === '') {
        ...
        const d = x.split(' to ')
        if (d.length > 1) {
          this.loadDataUtang(d[0], d[1])
        } else {
          this.loadDataUtang(d[0], d[0])
        }
      }
    },
    supplier(x) {
      if (x === null) {
        this.loadUtangSupplier(null)
      } else {
        this.loadUtangSupplier(x.id)
      }
    },
    statusFilter(x) {
      this.loadUtangStatus(x.value)
    },
    /* eslint-disable */
    searchQuery(query) {
      if (query === '') {
        ...
        this.listData = this.listTemp.filter(
          item => item.nomor_transaksi.toLowerCase().indexOf(query.toLowerCase()) > -1 || item.nama_supplier.toLowerCase().indexOf(query.toLowerCase()) > -1,
        )
      }
    },
    /* eslint-enable */
  ...
  methods: {
    showModalPembayaran(data) {
      this.$store.commit('app-transaksi-pembelian/SET_LIST_PEMBAYARAN', data.list_pembayaran)
      this.$bvModal.show('modal-daftar-pembayaran-pembelian')
    },
    showBulkPembayaran() {
      this.$emit('bulkPembayaran', this.listSupplier)
    },
    cekJatuhTempo(b) {
    ...
    loadDataUtang(dd = null, ddd = null) {
    ...
    loadDataSupplier(dd = null, ddd = null) {
      const dataUser = JSON.parse(localStorage.getItem('userData'))
      const cabang = dataUser.cabang_id
      store
        .dispatch('app-keuangan/getListSupplier', {
          cabang,
          dd,
          ddd,
        })
  ...
  setup() {
    const tableColumns = [
      ...
      { key: 'nama_supplier', sortable: false },
