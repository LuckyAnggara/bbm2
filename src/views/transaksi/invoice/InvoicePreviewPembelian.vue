  ...
  <section class="invoice-preview-wrapper">
    ...
    <b-alert variant="danger" v-if="dataInvoice === undefined ? true : false">
      ...
      <div class="alert-body">
        ...
        <b-link class="alert-link" :to="{ name: 'transaksi-pembelian-daftar' }">
    ...
    <b-row v-if="dataInvoice === undefined ? false : true" class="invoice-preview">
          ...
          <b-card-body class="invoice-padding pt-0">
            <b-row class="invoice-spacing">
              ...
              <b-col cols="12" xl="6" class="p-0">
                ...
                <h6 class="mb-25">
                  {{ dataInvoice.supplier.nama }}
                </h6>
                <p class="card-text mb-25">
                  <span style="white-space: pre-line;"> {{ dataInvoice.supplier.alamat }}</span>
                </p>

                <p class="card-text mb-25">
                  {{ dataInvoice.supplier.nomorTelepon }}
                </p>
              </b-col>
              <!-- Col: Payment Details -->
        ...
        <b-card>
          ...
          <div v-if="!typeRetur">
            ...
            <!-- <b-button
              ...
              class="mb-75"
              block
              :to="{ name: 'transaksi-pembelian-edit', params: { id: dataInvoice.id } }"
            >
              Edit
            </b-button> -->
            <hr />
      ...
      <modal-pembayaran :data-utang="dataInvoice" />
...
import ModalPembayaran from './component/ModalPembayaranUtang.vue'
import ModalDaftarPembayaran from './component/ModalDaftarPembayaranPembelian.vue'
...
export default {
  ...
  mounted() {
    ...
    if (id !== undefined) {
      store.dispatch('app-transaksi-pembelian/fetchTransaksi', id).then(res => {
        if (res.status === 200) {
          store.commit('app-transaksi-pembelian/SET_DATA_INVOICE', res.data)
          store.commit('app-transaksi-pembelian/SET_LIST_PEMBAYARAN', res.data.pembayaran.listPembayaran)
          this.dataInvoice = store.getters['app-transaksi-pembelian/getDataInvoice']
        }
      })
    }
  },
